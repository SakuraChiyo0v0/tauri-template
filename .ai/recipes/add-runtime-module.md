...
7. Use only the Host SDK version declared by the manifest. V1 provides logger, namespaced settings and theme; V2 adds module-isolated SQLite; V3 adds session-bound filesystem, process, registry, tray and shortcut proxies. Do not import base-app source files or call private Tauri commands.
...
9. Keep cleanup handles in module scope or element instances and release them from `deactivate()`/`disconnectedCallback()`.
