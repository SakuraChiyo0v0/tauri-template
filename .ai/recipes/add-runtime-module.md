...
7. Use only the Host SDK version declared by the manifest. V1 provides logger, namespaced settings and theme; V2 additionally provides the module-isolated SQLite API. Do not import base-app source files or call private Tauri commands.
...
10. Run `pnpm check` and `pnpm module:pack` inside the module repository, then install the generated `.mtp` from 模块管理.
